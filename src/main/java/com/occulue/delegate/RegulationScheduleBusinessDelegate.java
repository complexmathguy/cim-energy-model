 ...
 * <li>Improving the available of RegulationSchedule related services in the case of a RegulationSchedule business related service failing.</li>
 * <li>Exposes a simpler, uniform RegulationSchedule interface to the business tier, making it easy for clients to consume a simple Java object.</li>
 * <li>Hides the communication protocol that may be required to fulfill RegulationSchedule business related services.</li>
 * </ol>
 * <p>
 * @author your_name_here
 */
public class RegulationScheduleBusinessDelegate 
    ...
    */
    ...
    throws ProcessingException, IllegalArgumentException {
		...
		try {
    		...
    		// ---------------------------------------
    		// issue the CreateRegulationScheduleCommand - by convention the future return value for a create command
        	// that is handled by the constructor of an aggregate will return the UUID 
    		// ---------------------------------------
        	completableFuture = commandGateway.send( command );
        	
			LOGGER.log( Level.INFO, "return from Command Gateway for CreateRegulationScheduleCommand of RegulationSchedule is " + command );
    ...
    */
    ...
    throws ProcessingException, IllegalArgumentException {	
        ...
        try {  
            ...
            final String errMsg = "Unable to delete RegulationSchedule using Id = "  + command.getRegulationScheduleId();
            LOGGER.log( Level.WARNING, errMsg, exc );
            throw new ProcessingException( errMsg, exc );
        }
        finally {
    ...
    throws ProcessingException, IllegalArgumentException {
        ...
        try {
        	...
        	// --------------------------------------
        	CompletableFuture<RegulationSchedule> futureEntity = queryGateway.query(new FindRegulationScheduleQuery( new LoadRegulationScheduleFilter( summary.getRegulationScheduleId() ) ), ResponseTypes.instanceOf(RegulationSchedule.class));
        	
        	entity = futureEntity.get();
        }
        catch( Exception exc ) {
            final String errMsg = "Unable to locate RegulationSchedule with id " + summary.getRegulationScheduleId();
            LOGGER.log( Level.WARNING, errMsg, exc );
            throw new ProcessingException( errMsg, exc );
        }
        finally {
     ...
     */
    public List<RegulationSchedule> getAllRegulationSchedule() 
    throws ProcessingException {
        List<RegulationSchedule> list = null;

        try {
        	CompletableFuture<List<RegulationSchedule>> futureList = queryGateway.query(new FindAllRegulationScheduleQuery(), ResponseTypes.multipleInstancesOf(RegulationSchedule.class));
	 ...
	 */
	protected RegulationSchedule load( UUID id ) throws ProcessingException {
		regulationSchedule = RegulationScheduleBusinessDelegate.getRegulationScheduleInstance().getRegulationSchedule( new RegulationScheduleFetchOneSummary(id) );	
		return regulationSchedule;
	}
...
//************************************************************************
    ...
    private static final Logger LOGGER 			= Logger.getLogger(RegulationScheduleBusinessDelegate.class.getName());
