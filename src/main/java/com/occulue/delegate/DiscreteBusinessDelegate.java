 ...
 * <li>Improving the available of Discrete related services in the case of a Discrete business related service failing.</li>
 * <li>Exposes a simpler, uniform Discrete interface to the business tier, making it easy for clients to consume a simple Java object.</li>
 * <li>Hides the communication protocol that may be required to fulfill Discrete business related services.</li>
 * </ol>
 * <p>
 * @author your_name_here
 */
public class DiscreteBusinessDelegate 
    ...
    */
    ...
    throws ProcessingException, IllegalArgumentException {
		...
		try {
        	...
        	// -------------------------------------- 
			if ( command.getDiscreteId() == null )
				command.setDiscreteId( UUID.randomUUID() );
    		...
    		// ---------------------------------------
    		// issue the CreateDiscreteCommand - by convention the future return value for a create command
        	// that is handled by the constructor of an aggregate will return the UUID 
    		// ---------------------------------------
        	completableFuture = commandGateway.send( command );
        	
			LOGGER.log( Level.INFO, "return from Command Gateway for CreateDiscreteCommand of Discrete is " + command );
    ...
    */
    ...
    throws ProcessingException, IllegalArgumentException {
        ...
        try {
        	...
        	// --------------------------------------
        	CompletableFuture<Discrete> futureEntity = queryGateway.query(new FindDiscreteQuery( new LoadDiscreteFilter( summary.getDiscreteId() ) ), ResponseTypes.instanceOf(Discrete.class));
     ...
     */
    public List<Discrete> getAllDiscrete() 
    throws ProcessingException {
        List<Discrete> list = null;

        try {
        	CompletableFuture<List<Discrete>> futureList = queryGateway.query(new FindAllDiscreteQuery(), ResponseTypes.multipleInstancesOf(Discrete.class));
	 ...
	 * @return		Discrete
	 */
	protected Discrete load( UUID id ) throws ProcessingException {
		discrete = DiscreteBusinessDelegate.getDiscreteInstance().getDiscrete( new DiscreteFetchOneSummary(id) );	
		return discrete;
	}
