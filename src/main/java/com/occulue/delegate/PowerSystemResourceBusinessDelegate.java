 ...
 * <li>Improving the available of PowerSystemResource related services in the case of a PowerSystemResource business related service failing.</li>
 * <li>Exposes a simpler, uniform PowerSystemResource interface to the business tier, making it easy for clients to consume a simple Java object.</li>
 * <li>Hides the communication protocol that may be required to fulfill PowerSystemResource business related services.</li>
 * </ol>
 * <p>
 * @author your_name_here
 */
public class PowerSystemResourceBusinessDelegate 
    ...
    */
    ...
    throws ProcessingException, IllegalArgumentException {
		...
		try {
    		...
    		// ---------------------------------------
    		// issue the CreatePowerSystemResourceCommand - by convention the future return value for a create command
        	// that is handled by the constructor of an aggregate will return the UUID 
    		// ---------------------------------------
        	completableFuture = commandGateway.send( command );
        	
			LOGGER.log( Level.INFO, "return from Command Gateway for CreatePowerSystemResourceCommand of PowerSystemResource is " + command );
    ...
    */
    ...
    throws ProcessingException, IllegalArgumentException {	
        ...
        try {  
            ...
            final String errMsg = "Unable to delete PowerSystemResource using Id = "  + command.getPowerSystemResourceId();
            LOGGER.log( Level.WARNING, errMsg, exc );
            throw new ProcessingException( errMsg, exc );
        }
        finally {
    ...
    throws ProcessingException, IllegalArgumentException {
        ...
        try {
        	...
        	// --------------------------------------
        	CompletableFuture<PowerSystemResource> futureEntity = queryGateway.query(new FindPowerSystemResourceQuery( new LoadPowerSystemResourceFilter( summary.getPowerSystemResourceId() ) ), ResponseTypes.instanceOf(PowerSystemResource.class));
        	
        	entity = futureEntity.get();
        }
        catch( Exception exc ) {
            final String errMsg = "Unable to locate PowerSystemResource with id " + summary.getPowerSystemResourceId();
            LOGGER.log( Level.WARNING, errMsg, exc );
            throw new ProcessingException( errMsg, exc );
        }
        finally {
     ...
     */
    public List<PowerSystemResource> getAllPowerSystemResource() 
    throws ProcessingException {
        List<PowerSystemResource> list = null;

        try {
        	CompletableFuture<List<PowerSystemResource>> futureList = queryGateway.query(new FindAllPowerSystemResourceQuery(), ResponseTypes.multipleInstancesOf(PowerSystemResource.class));
	 ...
	 */
	protected PowerSystemResource load( UUID id ) throws ProcessingException {
		powerSystemResource = PowerSystemResourceBusinessDelegate.getPowerSystemResourceInstance().getPowerSystemResource( new PowerSystemResourceFetchOneSummary(id) );	
		return powerSystemResource;
	}
...
//************************************************************************
    ...
    private static final Logger LOGGER 			= Logger.getLogger(PowerSystemResourceBusinessDelegate.class.getName());
