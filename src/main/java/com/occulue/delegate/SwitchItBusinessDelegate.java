 ...
 * <li>Improving the available of SwitchIt related services in the case of a SwitchIt business related service failing.</li>
 * <li>Exposes a simpler, uniform SwitchIt interface to the business tier, making it easy for clients to consume a simple Java object.</li>
 * <li>Hides the communication protocol that may be required to fulfill SwitchIt business related services.</li>
 * </ol>
 * <p>
 * @author your_name_here
 */
public class SwitchItBusinessDelegate 
    ...
    */
    ...
    throws ProcessingException, IllegalArgumentException {
		...
		try {
        	...
        	// -------------------------------------- 
			if ( command.getSwitchItId() == null )
				command.setSwitchItId( UUID.randomUUID() );
    		...
    		// ---------------------------------------
    		// issue the CreateSwitchItCommand - by convention the future return value for a create command
        	// that is handled by the constructor of an aggregate will return the UUID 
    		// ---------------------------------------
        	completableFuture = commandGateway.send( command );
        	
			LOGGER.log( Level.INFO, "return from Command Gateway for CreateSwitchItCommand of SwitchIt is " + command );
    ...
    */
    ...
    throws ProcessingException, IllegalArgumentException {
        ...
        try {
        	...
        	// --------------------------------------
        	CompletableFuture<SwitchIt> futureEntity = queryGateway.query(new FindSwitchItQuery( new LoadSwitchItFilter( summary.getSwitchItId() ) ), ResponseTypes.instanceOf(SwitchIt.class));
     ...
     */
    public List<SwitchIt> getAllSwitchIt() 
    throws ProcessingException {
        List<SwitchIt> list = null;

        try {
        	CompletableFuture<List<SwitchIt>> futureList = queryGateway.query(new FindAllSwitchItQuery(), ResponseTypes.multipleInstancesOf(SwitchIt.class));
	 ...
	 * @return		SwitchIt
	 */
	protected SwitchIt load( UUID id ) throws ProcessingException {
		switchIt = SwitchItBusinessDelegate.getSwitchItInstance().getSwitchIt( new SwitchItFetchOneSummary(id) );	
		return switchIt;
	}
