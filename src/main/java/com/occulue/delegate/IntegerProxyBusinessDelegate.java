 ...
 * <li>Improving the available of IntegerProxy related services in the case of a IntegerProxy business related service failing.</li>
 * <li>Exposes a simpler, uniform IntegerProxy interface to the business tier, making it easy for clients to consume a simple Java object.</li>
 * <li>Hides the communication protocol that may be required to fulfill IntegerProxy business related services.</li>
 * </ol>
 * <p>
 * @author your_name_here
 */
public class IntegerProxyBusinessDelegate 
    ...
    */
    ...
    throws ProcessingException, IllegalArgumentException {
		...
		try {
        	...
        	// -------------------------------------- 
			if ( command.getIntegerProxyId() == null )
				command.setIntegerProxyId( UUID.randomUUID() );
    		...
    		// ---------------------------------------
    		// issue the CreateIntegerProxyCommand - by convention the future return value for a create command
        	// that is handled by the constructor of an aggregate will return the UUID 
    		// ---------------------------------------
        	completableFuture = commandGateway.send( command );
        	
			LOGGER.log( Level.INFO, "return from Command Gateway for CreateIntegerProxyCommand of IntegerProxy is " + command );
    ...
    */
    ...
    throws ProcessingException, IllegalArgumentException {	
        ...
        try {  
            ...
            final String errMsg = "Unable to delete IntegerProxy using Id = "  + command.getIntegerProxyId();
            LOGGER.log( Level.WARNING, errMsg, exc );
            throw new ProcessingException( errMsg, exc );
        }
        finally {
    ...
    throws ProcessingException, IllegalArgumentException {
        ...
        try {
        	...
        	// --------------------------------------
        	CompletableFuture<IntegerProxy> futureEntity = queryGateway.query(new FindIntegerProxyQuery( new LoadIntegerProxyFilter( summary.getIntegerProxyId() ) ), ResponseTypes.instanceOf(IntegerProxy.class));
     ...
     */
    public List<IntegerProxy> getAllIntegerProxy() 
    throws ProcessingException {
        List<IntegerProxy> list = null;

        try {
        	CompletableFuture<List<IntegerProxy>> futureList = queryGateway.query(new FindAllIntegerProxyQuery(), ResponseTypes.multipleInstancesOf(IntegerProxy.class));
	 ...
	 * @return		IntegerProxy
	 */
	protected IntegerProxy load( UUID id ) throws ProcessingException {
		integerProxy = IntegerProxyBusinessDelegate.getIntegerProxyInstance().getIntegerProxy( new IntegerProxyFetchOneSummary(id) );	
		return integerProxy;
	}
...
//************************************************************************
    ...
    private static final Logger LOGGER 			= Logger.getLogger(IntegerProxyBusinessDelegate.class.getName());
