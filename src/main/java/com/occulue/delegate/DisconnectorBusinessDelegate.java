 ...
 * <li>Improving the available of Disconnector related services in the case of a Disconnector business related service failing.</li>
 * <li>Exposes a simpler, uniform Disconnector interface to the business tier, making it easy for clients to consume a simple Java object.</li>
 * <li>Hides the communication protocol that may be required to fulfill Disconnector business related services.</li>
 * </ol>
 * <p>
 * @author your_name_here
 */
public class DisconnectorBusinessDelegate 
    ...
    */
    ...
    throws ProcessingException, IllegalArgumentException {
		...
		try {
        	...
        	// -------------------------------------- 
			if ( command.getDisconnectorId() == null )
				command.setDisconnectorId( UUID.randomUUID() );
    		...
    		// ---------------------------------------
    		// issue the CreateDisconnectorCommand - by convention the future return value for a create command
        	// that is handled by the constructor of an aggregate will return the UUID 
    		// ---------------------------------------
        	completableFuture = commandGateway.send( command );
        	
			LOGGER.log( Level.INFO, "return from Command Gateway for CreateDisconnectorCommand of Disconnector is " + command );
    ...
    */
    ...
    throws ProcessingException, IllegalArgumentException {	
        ...
        try {  
            ...
            final String errMsg = "Unable to delete Disconnector using Id = "  + command.getDisconnectorId();
            LOGGER.log( Level.WARNING, errMsg, exc );
            throw new ProcessingException( errMsg, exc );
        }
        finally {
    ...
    throws ProcessingException, IllegalArgumentException {
        ...
        try {
        	...
        	// --------------------------------------
        	CompletableFuture<Disconnector> futureEntity = queryGateway.query(new FindDisconnectorQuery( new LoadDisconnectorFilter( summary.getDisconnectorId() ) ), ResponseTypes.instanceOf(Disconnector.class));
     ...
     */
    public List<Disconnector> getAllDisconnector() 
    throws ProcessingException {
        List<Disconnector> list = null;

        try {
        	CompletableFuture<List<Disconnector>> futureList = queryGateway.query(new FindAllDisconnectorQuery(), ResponseTypes.multipleInstancesOf(Disconnector.class));
	 ...
	 * @return		Disconnector
	 */
	protected Disconnector load( UUID id ) throws ProcessingException {
		disconnector = DisconnectorBusinessDelegate.getDisconnectorInstance().getDisconnector( new DisconnectorFetchOneSummary(id) );	
		return disconnector;
	}
...
//************************************************************************
    ...
    private static final Logger LOGGER 			= Logger.getLogger(DisconnectorBusinessDelegate.class.getName());
