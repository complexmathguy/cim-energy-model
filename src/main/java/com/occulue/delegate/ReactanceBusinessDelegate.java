 ...
 * <li>Improving the available of Reactance related services in the case of a Reactance business related service failing.</li>
 * <li>Exposes a simpler, uniform Reactance interface to the business tier, making it easy for clients to consume a simple Java object.</li>
 * <li>Hides the communication protocol that may be required to fulfill Reactance business related services.</li>
 * </ol>
 * <p>
 * @author your_name_here
 */
public class ReactanceBusinessDelegate 
    ...
    */
    ...
    throws ProcessingException, IllegalArgumentException {
		...
		try {
        	...
        	// -------------------------------------- 
			if ( command.getReactanceId() == null )
				command.setReactanceId( UUID.randomUUID() );
    		...
    		// ---------------------------------------
    		// issue the CreateReactanceCommand - by convention the future return value for a create command
        	// that is handled by the constructor of an aggregate will return the UUID 
    		// ---------------------------------------
        	completableFuture = commandGateway.send( command );
        	
			LOGGER.log( Level.INFO, "return from Command Gateway for CreateReactanceCommand of Reactance is " + command );
    ...
    */
    ...
    throws ProcessingException, IllegalArgumentException {
        ...
        try {
        	...
        	// --------------------------------------
        	CompletableFuture<Reactance> futureEntity = queryGateway.query(new FindReactanceQuery( new LoadReactanceFilter( summary.getReactanceId() ) ), ResponseTypes.instanceOf(Reactance.class));
     ...
     */
    public List<Reactance> getAllReactance() 
    throws ProcessingException {
        List<Reactance> list = null;

        try {
        	CompletableFuture<List<Reactance>> futureList = queryGateway.query(new FindAllReactanceQuery(), ResponseTypes.multipleInstancesOf(Reactance.class));
	 ...
	 * @return		Reactance
	 */
	protected Reactance load( UUID id ) throws ProcessingException {
		reactance = ReactanceBusinessDelegate.getReactanceInstance().getReactance( new ReactanceFetchOneSummary(id) );	
		return reactance;
	}
