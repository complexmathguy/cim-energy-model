 ...
 * <li>Improving the available of Junction related services in the case of a Junction business related service failing.</li>
 * <li>Exposes a simpler, uniform Junction interface to the business tier, making it easy for clients to consume a simple Java object.</li>
 * <li>Hides the communication protocol that may be required to fulfill Junction business related services.</li>
 * </ol>
 * <p>
 * @author your_name_here
 */
public class JunctionBusinessDelegate 
    ...
    */
    ...
    throws ProcessingException, IllegalArgumentException {
		...
		try {
        	...
        	// -------------------------------------- 
			if ( command.getJunctionId() == null )
				command.setJunctionId( UUID.randomUUID() );
    		...
    		// ---------------------------------------
    		// issue the CreateJunctionCommand - by convention the future return value for a create command
        	// that is handled by the constructor of an aggregate will return the UUID 
    		// ---------------------------------------
        	completableFuture = commandGateway.send( command );
        	
			LOGGER.log( Level.INFO, "return from Command Gateway for CreateJunctionCommand of Junction is " + command );
    ...
    */
    ...
    throws ProcessingException, IllegalArgumentException {
        ...
        try {
        	...
        	// --------------------------------------
        	CompletableFuture<Junction> futureEntity = queryGateway.query(new FindJunctionQuery( new LoadJunctionFilter( summary.getJunctionId() ) ), ResponseTypes.instanceOf(Junction.class));
     ...
     */
    public List<Junction> getAllJunction() 
    throws ProcessingException {
        List<Junction> list = null;

        try {
        	CompletableFuture<List<Junction>> futureList = queryGateway.query(new FindAllJunctionQuery(), ResponseTypes.multipleInstancesOf(Junction.class));
	 ...
	 * @return		Junction
	 */
	protected Junction load( UUID id ) throws ProcessingException {
		junction = JunctionBusinessDelegate.getJunctionInstance().getJunction( new JunctionFetchOneSummary(id) );	
		return junction;
	}
