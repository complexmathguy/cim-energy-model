 ...
 * <li>Improving the available of DCGround related services in the case of a DCGround business related service failing.</li>
 * <li>Exposes a simpler, uniform DCGround interface to the business tier, making it easy for clients to consume a simple Java object.</li>
 * <li>Hides the communication protocol that may be required to fulfill DCGround business related services.</li>
 * </ol>
 * <p>
 * @author your_name_here
 */
public class DCGroundBusinessDelegate 
    ...
    */
    ...
    throws ProcessingException, IllegalArgumentException {
		...
		try {
        	...
        	// -------------------------------------- 
			if ( command.getDCGroundId() == null )
				command.setDCGroundId( UUID.randomUUID() );
    		...
    		// ---------------------------------------
    		// issue the CreateDCGroundCommand - by convention the future return value for a create command
        	// that is handled by the constructor of an aggregate will return the UUID 
    		// ---------------------------------------
        	completableFuture = commandGateway.send( command );
        	
			LOGGER.log( Level.INFO, "return from Command Gateway for CreateDCGroundCommand of DCGround is " + command );
    ...
    */
    ...
    throws ProcessingException, IllegalArgumentException {
        ...
        try {
        	...
        	// --------------------------------------
        	CompletableFuture<DCGround> futureEntity = queryGateway.query(new FindDCGroundQuery( new LoadDCGroundFilter( summary.getDCGroundId() ) ), ResponseTypes.instanceOf(DCGround.class));
     ...
     */
    public List<DCGround> getAllDCGround() 
    throws ProcessingException {
        List<DCGround> list = null;

        try {
        	CompletableFuture<List<DCGround>> futureList = queryGateway.query(new FindAllDCGroundQuery(), ResponseTypes.multipleInstancesOf(DCGround.class));
	 ...
	 * @return		DCGround
	 */
	protected DCGround load( UUID id ) throws ProcessingException {
		dCGround = DCGroundBusinessDelegate.getDCGroundInstance().getDCGround( new DCGroundFetchOneSummary(id) );	
		return dCGround;
	}
