 ...
 * <li>Improving the available of WindMechIEC related services in the case of a WindMechIEC business related service failing.</li>
 * <li>Exposes a simpler, uniform WindMechIEC interface to the business tier, making it easy for clients to consume a simple Java object.</li>
 * <li>Hides the communication protocol that may be required to fulfill WindMechIEC business related services.</li>
 * </ol>
 * <p>
 * @author your_name_here
 */
public class WindMechIECBusinessDelegate 
    ...
    */
    ...
    throws ProcessingException, IllegalArgumentException {
		...
		try {
        	...
        	// -------------------------------------- 
			if ( command.getWindMechIECId() == null )
				command.setWindMechIECId( UUID.randomUUID() );
    		...
    		// ---------------------------------------
    		// issue the CreateWindMechIECCommand - by convention the future return value for a create command
        	// that is handled by the constructor of an aggregate will return the UUID 
    		// ---------------------------------------
        	completableFuture = commandGateway.send( command );
        	
			LOGGER.log( Level.INFO, "return from Command Gateway for CreateWindMechIECCommand of WindMechIEC is " + command );
    ...
    */
    ...
    throws ProcessingException, IllegalArgumentException {
        ...
        try {
        	...
        	// --------------------------------------
        	CompletableFuture<WindMechIEC> futureEntity = queryGateway.query(new FindWindMechIECQuery( new LoadWindMechIECFilter( summary.getWindMechIECId() ) ), ResponseTypes.instanceOf(WindMechIEC.class));
     ...
     */
    public List<WindMechIEC> getAllWindMechIEC() 
    throws ProcessingException {
        List<WindMechIEC> list = null;

        try {
        	CompletableFuture<List<WindMechIEC>> futureList = queryGateway.query(new FindAllWindMechIECQuery(), ResponseTypes.multipleInstancesOf(WindMechIEC.class));
	 ...
	 * @return		WindMechIEC
	 */
	protected WindMechIEC load( UUID id ) throws ProcessingException {
		windMechIEC = WindMechIECBusinessDelegate.getWindMechIECInstance().getWindMechIEC( new WindMechIECFetchOneSummary(id) );	
		return windMechIEC;
	}
...
//************************************************************************
    ...
    private static final Logger LOGGER 			= Logger.getLogger(WindMechIECBusinessDelegate.class.getName());
