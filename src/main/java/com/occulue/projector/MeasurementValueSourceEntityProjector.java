...
public class MeasurementValueSourceEntityProjector {
    ...
    public List<MeasurementValueSource> findAll( FindAllMeasurementValueSourceQuery query ) {
    	...
    	try {
    		...
    		LOGGER.log( Level.WARNING, "Failed to find all MeasurementValueSource - {0}", exc.getMessage() );
    	}
    	return null;
    }
    ...
    // --------------------------------------------------
	@Autowired
    protected final MeasurementValueSourceRepository repository;

    private static final Logger LOGGER 	= Logger.getLogger(MeasurementValueSourceEntityProjector.class.getName());
