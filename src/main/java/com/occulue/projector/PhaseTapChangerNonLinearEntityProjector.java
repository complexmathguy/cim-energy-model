...
public class PhaseTapChangerNonLinearEntityProjector {
    ...
    public PhaseTapChangerNonLinear find( UUID id ) {
    	...
    	try {
    		...
    		LOGGER.log( Level.WARNING, "Failed to find a PhaseTapChangerNonLinear - {0}", exc.getMessage() );
    	}
    	return null;
    }
    ...
    public List<PhaseTapChangerNonLinear> findAll( FindAllPhaseTapChangerNonLinearQuery query ) {
    	...
    	try {
    		...
    		LOGGER.log( Level.WARNING, "Failed to find all PhaseTapChangerNonLinear - {0}", exc.getMessage() );
    	}
    	return null;
    }
    ...
    // --------------------------------------------------
	@Autowired
    protected final PhaseTapChangerNonLinearRepository repository;

    private static final Logger LOGGER 	= Logger.getLogger(PhaseTapChangerNonLinearEntityProjector.class.getName());
