...
public class MechanicalLoadDynamicsEntityProjector {
    ...
    public List<MechanicalLoadDynamics> findAll( FindAllMechanicalLoadDynamicsQuery query ) {
    	...
    	try {
    		...
    		LOGGER.log( Level.WARNING, "Failed to find all MechanicalLoadDynamics - {0}", exc.getMessage() );
    	}
    	return null;
    }
    ...
    // --------------------------------------------------
	@Autowired
    protected final MechanicalLoadDynamicsRepository repository;

    private static final Logger LOGGER 	= Logger.getLogger(MechanicalLoadDynamicsEntityProjector.class.getName());
