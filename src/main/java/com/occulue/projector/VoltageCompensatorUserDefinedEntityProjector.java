...
public class VoltageCompensatorUserDefinedEntityProjector {
	...
	// core constructor
	public VoltageCompensatorUserDefinedEntityProjector(VoltageCompensatorUserDefinedRepository repository ) {
    ...
    /**
     * Method to retrieve the VoltageCompensatorUserDefined via an FindVoltageCompensatorUserDefinedQuery
    ...
    public VoltageCompensatorUserDefined find( UUID id ) {
    	...
    	try {
    		...
    		LOGGER.log( Level.WARNING, "Failed to find a VoltageCompensatorUserDefined - {0}", exc.getMessage() );
    	}
    	return null;
    }
     ...
     */
    @SuppressWarnings("unused")
    public List<VoltageCompensatorUserDefined> findAll( FindAllVoltageCompensatorUserDefinedQuery query ) {
    	LOGGER.info("handling findAll using " + query.toString() );
    	try {
    		return repository.findAll();
    	}
    	catch( IllegalArgumentException exc ) {
    		LOGGER.log( Level.WARNING, "Failed to find all VoltageCompensatorUserDefined - {0}", exc.getMessage() );
    	}
    	return null;
    }
    ...
    // --------------------------------------------------
	@Autowired
    protected final VoltageCompensatorUserDefinedRepository repository;

    private static final Logger LOGGER 	= Logger.getLogger(VoltageCompensatorUserDefinedEntityProjector.class.getName());
