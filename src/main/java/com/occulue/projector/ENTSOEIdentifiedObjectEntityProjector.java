...
public class ENTSOEIdentifiedObjectEntityProjector {
    ...
    public List<ENTSOEIdentifiedObject> findAll( FindAllENTSOEIdentifiedObjectQuery query ) {
    	...
    	try {
    		...
    		LOGGER.log( Level.WARNING, "Failed to find all ENTSOEIdentifiedObject - {0}", exc.getMessage() );
    	}
    	return null;
    }
    ...
    // --------------------------------------------------
	@Autowired
    protected final ENTSOEIdentifiedObjectRepository repository;

    private static final Logger LOGGER 	= Logger.getLogger(ENTSOEIdentifiedObjectEntityProjector.class.getName());
