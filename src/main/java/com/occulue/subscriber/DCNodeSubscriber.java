...
public class DCNodeSubscriber extends BaseSubscriber {
    ...
    public SubscriptionQueryResult<List<DCNode>, DCNode> dCNodeSubscribe() {
        return queryGateway
                .subscriptionQuery(new FindAllDCNodeQuery(), 
                		ResponseTypes.multipleInstancesOf(DCNode.class),
                		ResponseTypes.instanceOf(DCNode.class));
    }

    public SubscriptionQueryResult<DCNode, DCNode> dCNodeSubscribe(@DestinationVariable UUID dCNodeId) {
        return queryGateway
                .subscriptionQuery(new FindDCNodeQuery(new LoadDCNodeFilter(dCNodeId)), 
                		ResponseTypes.instanceOf(DCNode.class),
                		ResponseTypes.instanceOf(DCNode.class));
    }
