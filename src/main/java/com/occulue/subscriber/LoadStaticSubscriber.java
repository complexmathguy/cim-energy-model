...
public class LoadStaticSubscriber extends BaseSubscriber {
    ...
    public SubscriptionQueryResult<List<LoadStatic>, LoadStatic> loadStaticSubscribe() {
        return queryGateway
                .subscriptionQuery(new FindAllLoadStaticQuery(), 
                		ResponseTypes.multipleInstancesOf(LoadStatic.class),
                		ResponseTypes.instanceOf(LoadStatic.class));
    }

    public SubscriptionQueryResult<LoadStatic, LoadStatic> loadStaticSubscribe(@DestinationVariable UUID loadStaticId) {
        return queryGateway
                .subscriptionQuery(new FindLoadStaticQuery(new LoadLoadStaticFilter(loadStaticId)), 
                		ResponseTypes.instanceOf(LoadStatic.class),
                		ResponseTypes.instanceOf(LoadStatic.class));
    }
