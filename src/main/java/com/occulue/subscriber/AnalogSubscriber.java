...
public class AnalogSubscriber extends BaseSubscriber {
    ...
    public SubscriptionQueryResult<List<Analog>, Analog> analogSubscribe() {
        return queryGateway
                .subscriptionQuery(new FindAllAnalogQuery(), 
                		ResponseTypes.multipleInstancesOf(Analog.class),
                		ResponseTypes.instanceOf(Analog.class));
    }

    public SubscriptionQueryResult<Analog, Analog> analogSubscribe(@DestinationVariable UUID analogId) {
        return queryGateway
                .subscriptionQuery(new FindAnalogQuery(new LoadAnalogFilter(analogId)), 
                		ResponseTypes.instanceOf(Analog.class),
                		ResponseTypes.instanceOf(Analog.class));
    }
