...
public class ReactanceSubscriber extends BaseSubscriber {
    ...
    public SubscriptionQueryResult<List<Reactance>, Reactance> reactanceSubscribe() {
        return queryGateway
                .subscriptionQuery(new FindAllReactanceQuery(), 
                		ResponseTypes.multipleInstancesOf(Reactance.class),
                		ResponseTypes.instanceOf(Reactance.class));
    }

    public SubscriptionQueryResult<Reactance, Reactance> reactanceSubscribe(@DestinationVariable UUID reactanceId) {
        return queryGateway
                .subscriptionQuery(new FindReactanceQuery(new LoadReactanceFilter(reactanceId)), 
                		ResponseTypes.instanceOf(Reactance.class),
                		ResponseTypes.instanceOf(Reactance.class));
    }
