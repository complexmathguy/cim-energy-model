...
public class GovHydroPIDCommandRestController extends BaseSpringRestController {
     ...
     */
	@PostMapping("/create")
    public CompletableFuture<UUID> create( @RequestBody(required=true) CreateGovHydroPIDCommand command ) {
		CompletableFuture<UUID> completableFuture = null;
		try {       
        	
			completableFuture = GovHydroPIDBusinessDelegate.getGovHydroPIDInstance().createGovHydroPID( command );
        }
        catch( Throwable exc ) {
     ...
     */
	@PutMapping("/update")
    public CompletableFuture<Void> update( @RequestBody(required=true) UpdateGovHydroPIDCommand command ) {
		CompletableFuture<Void> completableFuture = null;
		try {                        	        
			// -----------------------------------------------
			// delegate the UpdateGovHydroPIDCommand
			// -----------------------------------------------
			completableFuture = GovHydroPIDBusinessDelegate.getGovHydroPIDInstance().updateGovHydroPID(command);;
	    }
	    catch( Throwable exc ) {
	    	LOGGER.log( Level.WARNING, "GovHydroPIDController:update() - successfully update GovHydroPID - " + exc.getMessage());        	
	    }		
    ...
    public CompletableFuture<Void> delete( @RequestParam(required=true) UUID govHydroPIDId  ) {
    	...
    	try {
    		...
    		LOGGER.log( Level.WARNING, "Successfully deleted GovHydroPID with key " + command.getGovHydroPIDId() );
        }
        catch( Throwable exc ) {
...
//************************************************************************
    ...
    private static final Logger LOGGER = Logger.getLogger(GovHydroPIDCommandRestController.class.getName());
