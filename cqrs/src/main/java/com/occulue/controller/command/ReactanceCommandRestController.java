...
public class ReactanceCommandRestController extends BaseSpringRestController {
     ...
     */
	@PostMapping("/create")
    public CompletableFuture<UUID> create( @RequestBody(required=true) CreateReactanceCommand command ) {
     ...
     */
	@PutMapping("/update")
    public CompletableFuture<Void> update( @RequestBody(required=true) UpdateReactanceCommand command ) {
		...
		try {                        	        
	    	...
	    	LOGGER.log( Level.WARNING, "ReactanceController:update() - successfully update Reactance - " + exc.getMessage());        	
	    }		
    ...
    public CompletableFuture<Void> delete( @RequestParam(required=true) UUID reactanceId  ) {
    	...
    	try {
    		...
    		LOGGER.log( Level.WARNING, "Successfully deleted Reactance with key " + command.getReactanceId() );
        }
        catch( Throwable exc ) {
...
//************************************************************************
    ...
    private static final Logger LOGGER = Logger.getLogger(ReactanceCommandRestController.class.getName());
